...
def do_zip_update():
    ...
    if success:
        ...
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/devilboy66/Bomber.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/devilboy66/Bomber.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
            "https://github.com/devilboy66/Bomber/blob/master/.version"
            ).text.strip()
    if fver != __VERSION__:
...
def notifyen():
    try:
        noti = requests.get(
            "https://github.com/devilboy66/Bomber/blob/master/.notify"
            ).text.upper()
        if len(noti) > 10:
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("TBomb was created by Devilboy")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 100000, "call": 15000, "mail": 20000}
        cc, target = "", ""
        if mode in ["sms", "call"]:
        ...
        while True:
            try:
                ...
                delay = float(input(
                    ...
                    .strip())
                # delay = 0
                max_thread_limit = (count//100) if (count//10) > 0 else 1
...
__CONTRIBUTORS__ = ['Devilboy6' , 'Gopal']

ALL_COLORS = [Fore.GREEN, Fore.RED, Fore.YELLOW, Fore.BLUE,
              Fore.MAGENTA, Fore.CYAN, Fore.WHITE]
RESET_ALL = Style.RESET_ALL

description = """Devilboy - Your Friendly Spammer Application
...
parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by Devilboy6 !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start Bombing with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start  Bombing  with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start  Bombing  with MAIL Bomb mode")
parser.add_argument("-u", "--update", action="store_true",
                    help="update  Bombing ")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current  Bombing  contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current  Bombing version")
